...
def gerar_pdf_custos_detalhado():
    ...
    # --- LÓGICA DE CORREÇÃO DA POTÊNCIA (MANUAL VS AUTOMÁTICO) ---
    modo_manual_ativo = st.session_state.get('orc_man_key') or st.session_state.get('orc_man_disabled')
    if modo_manual_ativo:
    ...
    # 3. PERFORMANCE E GRÁFICO (CORRIGIDO PARA GERAÇÃO MANUAL)
    draw_section("3. PERFORMANCE MENSAL (kWh)")

    # Lógica de Geração: Manual vs Automática
    geracao_manual_ativa = st.session_state.get('geracao_manual_key', False)
    ...
    # Cálculo dinâmico de escala para o gráfico não estourar a margem
    max_valor_grafico = max(max(lista_geracao), m_c, 1.0)
    esc = 40.0 / max_valor_grafico  # Ajusta a altura máxima das barras para 40 unidades

    h_c = st.session_state.get('historico_12_meses_key', {})

    for i, m in enumerate(ms):
        vc = float(h_c.get(m, m_c))
        vg = lista_geracao[i]  # Usa o valor da lista (seja manual ou automático)

        can.setFillColor(colors.navy)
        can.rect(x_g + (i * 38), y_b, 12, vc * esc, fill=1, stroke=0)

        # Corrigindo cor laranja (navy blue e branco são as cores padrão, mas mantive orange para destaque do gráfico conforme original)
