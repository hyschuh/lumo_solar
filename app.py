...
if 'pagina' not in st.session_state:
    st.session_state.pagina = 'formulario'
def ir_para_proxima():
    # "Ampliação" agora faz parte dos gatilhos para o manual.py
...
bg_64 = get_base64(r"C:\Users\hysch\AppSolar\background.jpg")
logo_64 = get_base64(r"C:\Users\hysch\AppSolar\logo.jpg")
...
try:
    ...
    df_inversores = pd.read_excel(r"C:\Users\hysch\AppSolar\precos_lumo.xlsx", sheet_name="Inversores")
...
if not definir_modulo_manual:
    try:
        ...
        if marca_selecionada:
            df_modulo = pd.read_excel(r"C:\Users\hysch\AppSolar\precos_lumo.xlsx", sheet_name="Módulo")
...
# 1. Recuperar variáveis necessárias
tipo_telhado = st.session_state.get('tipo_telhado_key')
X = st.session_state.get('qtd_modulos_final', 0)

# 2. Casos que não geram cálculo automático
if tipo_telhado in ["Laje", "Solo", "Kalheta"]:
    st.session_state['custo_estrutura_final'] = 0.0

# 3. Lógica para Fibrocimento, Metálico ou Telha
else:
    try:
        df_estruturas = pd.read_excel(r"C:\Users\hysch\AppSolar\precos_lumo.xlsx", sheet_name="Estruturas")
        precos = dict(zip(df_estruturas.iloc[:, 0], df_estruturas.iloc[:, 1]))

        # Checkbox Estrutura Metálica
        ...
        # --- CÁLCULO AUTOMÁTICO INICIAL (Valores Sugeridos) ---
        if X > 0:
            Y = X / 2
        ...
        st.markdown(f'<p style="color: #ffa500; font-weight: bold;">Valor total de Estruturas ({total_estrutura})</p>',
                    unsafe_allow_html=True)
    except Exception as e:
...
try:
    caminho_excel = r"C:\Users\hysch\AppSolar\precos_lumo.xlsx"
...
# 1. Resgate de Variáveis de Controle e Custos
...
# 2. Somatória Base (Serviços + Lucro + Representante + Estrutura + Indicação se valor)
    ...
    with col_d1:
        st.markdown(f"""
            <div style="background-color: rgba(255, 255, 255, 0.1); padding: 15px; border-radius: 10px; border-left: 5px solid #ffa500; height: 130px;">
                ...
                <hr style="margin: 10px 0; border: 0.5px solid rgba(255,255,255,0.2);">
                <p style="color: white; font-size: 1.5em; font-weight: bold; margin: 0;">R$ {val_final_imposto:,.2f}</p>
            </div>
        """, unsafe_allow_html=True)
...
def buscar_imagem_logo(marca):
    diretorio_logos = r"C:\Users\hysch\AppSolar"
...
def renderizar_pagina_3(can):
    ...
    """
    caminho_fundo_p3 = r"C:\Users\hysch\AppSolar\tres.png"
...
def gerar_pdf_completo():
    writer = PdfWriter()
    caminho_capa = r"C:\Users\hysch\AppSolar\capa.pdf"
    caminho_fundo_p2 = r"C:\Users\hysch\AppSolar\dois.jpg"
    caminho_excel = r"C:\Users\hysch\AppSolar\precos_lumo.xlsx"

    try:
        # PÁGINA 1: CAPA (PDF Externo)
