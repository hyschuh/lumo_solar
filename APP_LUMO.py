...
#______________________________________________________________________________________-
    ...
    with st.expander("DADOS DO CLIENTE", expanded=True):
        ...
        if not atendimento_local:
            ...
            with col_cid2:
                st.number_input("Distância (km)", min_value=0.0, step=1.0, key='distancia_key')
...
# --- SEÇÃO 9: DIMENSIONAMENTO DE USINA (ATUALIZADA) ---

# 1. Recupera os dados da Seção 5 e 6 com tratamento de erro
try:
    consumo_base_calculo = float(st.session_state.get('consumo_final_key', 0.0))
except:
    consumo_base_calculo = 0.0

# 2. Lógica da Sobra de Geração
valor_sobra = 0.0
...
# 4. Lógica do Módulo e Quantidade Manual
...
if modo_manual_geral:
    ...
    if definir_modulo_manual:
        ...
        with col_man2:
            # NOVO CAMPO: Quantidade Manual
            qtd_manual = st.number_input("Quantidade de módulos", min_value=0, step=1,
                                         key='qtd_modulos_manual_key')
    else:
...
# 5. Busca na Planilha (Se não for inserção manual da potência)
...
# 6. Dimensionamento Final e Exibição
if potencia_final > 0:
    # Lógica de decisão: Se houver valor no campo manual e ele for > 0, usa ele. Caso contrário, calcula.
    if qtd_manual and qtd_manual > 0:
        qtd_modulos = int(qtd_manual)
        metodo_texto = "Definido Manualmente"
    else:
        ...
        metodo_texto = "Quantidade Necessária"
    ...
    with col_res2:
        st.markdown(f"""
            <div style="background-color: rgba(255, 255, 255, 0.1); padding: 15px; border-radius: 10px; border-left: 5px solid #ffa500; height: 160px;">
                <p style="color: white; margin: 0;">{metodo_texto}:</p>
...
# --- ADIÇÃO: GERAÇÃO MANUAL ---
geracao_manual_ativa = st.checkbox("Geração manual", key="geracao_manual_key")
dados_geracao_manual = {}

if geracao_manual_ativa:
    cols = st.columns(4)
    meses_lista = ["JAN", "FEV", "MAR", "ABR", "MAI", "JUN", "JUL", "AGO", "SET", "OUT", "NOV", "DEZ"]
    for i, m in enumerate(meses_lista):
        with cols[i % 4]:
            dados_geracao_manual[m] = st.number_input(f"Geração {m} (kWh)", min_value=0.0, step=1.0,
                                                      key=f"input_ger_{m}")
# ------------------------------
...
for mes in ordem_meses:
    ...
    # Geração (Lógica de prioridade: Manual se ativo, senão Automático)
    if geracao_manual_ativa:
        val_geracao = dados_geracao_manual.get(mes, 0.0)
    else:
        val_geracao = potencia_total_w * produtividade_mensal[mes]

    # Salva no session_state para uso no Payback posterior
    st.session_state[f'geracao_mensal_{mes}'] = val_geracao
...
# 9. Exibição final
if potencia_total_w > 0 or geracao_manual_ativa:
...
# --- NOVO: CÁLCULO DE DESLOCAMENTO (R$ 1,00 por km) ---
distancia_km = st.session_state.get('distancia_key', 0.0)
custo_deslocamento = distancia_km * 1.00

# 2. Cálculo do Valor Bruto (Incluindo o deslocamento agora)
valor_bruto = (
    v_kit + c_inst + c_mat + c_proj + c_cest +
    c_lucr + c_repr + c_estru + v_imp + v_ind + custo_deslocamento
)

# 3. Lógica de Arredondamento para cima (Centenas)
valor_arredondado = math.ceil(valor_bruto / 100) * 100

# Trata a exceção específica para valores próximos de milhar (Ex: 15.990)
...
# Guardar valor total arredondado para uso no PDF/Seção 18
st.session_state['valor_total_final_projeto'] = valor_arredondado

# --- SEÇÃO 18: RETORNO FINANCEIRO (ATUALIZADA COM LÓGICA RURAL E UNIFICAÇÃO) ---
import pandas as pd
import os
...
# Geração Média Mensal (Cálculo Automático)
media_geracao_kwh_auto = (potencia_total_w * sum(produtividade_mensal.values())) / 12

# Verificação de Geração Manual
geracao_manual_ativa = st.session_state.get('geracao_manual_key', False)
if geracao_manual_ativa:
    meses_ref = ["JAN", "FEV", "MAR", "ABR", "MAI", "JUN", "JUL", "AGO", "SET", "OUT", "NOV", "DEZ"]
    soma_man = sum([float(st.session_state.get(f"input_ger_{m}", 0)) for m in meses_ref])
    media_geracao_kwh = soma_man / 12
else:
    media_geracao_kwh = media_geracao_kwh_auto
...
try:
    ...
    if os.path.exists(caminho_excel):
        ...
        porcentagem_economia = (economia_mensal_nominal / valor_fatura_sem_solar * 100) if valor_fatura_sem_solar > 0 else 0.0
        ...
        # --- SALVAMENTO NO SESSION STATE (ESSENCIAL PARA O PDF) ---
        st.session_state['valor_fatura_sem_solar'] = valor_fatura_sem_solar
        st.session_state['economia_mensal_nominal'] = economia_mensal_nominal
        st.session_state['valor_fatura_estimada_solar'] = valor_fatura_estimada_solar
        st.session_state['payback_anos'] = payback_anos
        st.session_state['payback_meses'] = payback_meses
        st.session_state['media_geracao_final_pdf'] = media_geracao_kwh
...
# --- SEÇÃO 19: GERAÇÃO DE ORÇAMENTO EM PDF (COM UNIFICAÇÃO DE VALORES) ---
import io
import os
import re
import math
...
def buscar_imagem_logo(marca):
    ...
    if not marca: return None
    for ext in extensoes:
        caminho_teste = os.path.join(diretorio_logos, f"{marca.upper()}{ext}")
...
def renderizar_pagina_3(can):
    ...
    # --- RESGATE DA FONTE ÚNICA (DA SEÇÃO 18) ---
    valor_fatura_sem_solar = st.session_state.get('valor_fatura_sem_solar', 0.0)
    economia_mensal_nominal = st.session_state.get('economia_mensal_nominal', 0.0)
    valor_fatura_estimada_solar = st.session_state.get('valor_fatura_estimada_solar', 0.0)
    payback_meses_total = st.session_state.get('payback_meses', 0.0)
    valor_total_projeto = st.session_state.get('valor_total_final_projeto', 0.0)
    geracao_media_estimada = st.session_state.get('media_geracao_final_pdf', 0.0)

    # --- DESENHO DA LOGO (AGORA NA PÁGINA 3) ---
    logo = buscar_imagem_logo(marca_inv)
    if logo:
        can.drawImage(logo, 50, 20, width=240, height=120, mask='auto', preserveAspectRatio=True)

    # Potência Total
    pot_kwp = (qtd_mod * pot_mod_w) / 1000
    ...
    can.setFont("Helvetica-Bold", 12)
    can.drawCentredString(441, 536, f"{int(qtd_inv)}")
    ...
    # --- LÓGICA DE PAYBACK ---
    total_meses_arredondado = int(round(payback_meses_total))
    anos_inteiros, meses_restantes = divmod(total_meses_arredondado, 12)

    if anos_inteiros == 0:
        texto_anos = ""
    elif anos_inteiros == 1:
        texto_anos = "1 ano"
    else:
        texto_anos = f"{anos_inteiros} anos"
    ...
    if meses_restantes == 0:
        ...
        texto_meses = f"{meses_restantes} meses"

    if texto_anos and texto_meses:
        linha1, linha2 = texto_anos, f"e {texto_meses}"
    elif texto_anos:
        linha1, linha2 = texto_anos, ""
    elif texto_meses:
        linha1, linha2 = texto_meses, ""
    else:
        linha1, linha2 = "Imediato", ""

    if linha2 == "":
        can.drawCentredString(441, 217, linha1)
    else:
        can.drawCentredString(441, 224, linha1)
        can.drawCentredString(441, 210, linha2)

    can.setFont("Helvetica-Bold", 36)
    can.setFillColorRGB(1, 1, 1)
    can.drawRightString(550, 53, f"R$ {f_br(valor_total_projeto)}")
    can.showPage()
...
def gerar_pdf_completo():
    ...
    try:
        ...
        if os.path.exists(caminho_fundo_p2):
            ...
            data_atual = data_dt.strftime("%d/%m/%Y")
            data_validade = (data_dt + timedelta(days=5)).strftime("%d/%m/%Y")

            can.setFont("Helvetica-Bold", 18)
            can.setFillColorRGB(1, 1, 1)
            # Data de Emissão (Já existente)
            can.drawCentredString(470, 33, f"Data {data_atual}")

            # Nova Data de Validade (Conforme solicitado)
            can.setFont("Helvetica-Bold", 14)  # Fonte menor para caber no rodapé
            can.drawCentredString(470, 10, f"Validade até {data_validade}")
            ...
            can.setFillColorRGB(0, 0, 0)
            can.drawString(103, 603,
                           f"{qtd_inv} {'Inversor' if qtd_inv == 1 else 'Inversores'} {pot_inv} kW {marca_inv}")
            can.drawString(103, 563, f"{qtd_mod} {'Módulo' if qtd_mod == 1 else 'Módulos'} {int(pot_mod_w)}W")
            can.drawString(320, 608, f"Inversor com garantia de {garantia_inv} anos.")
            can.drawString(320, 563, f"Módulos com {garantia_mod} anos de garantia.")
            can.drawString(320, 520, "Garantia de 85% de eficiência em 30 anos")

            # --- LÓGICA DE SOBRA DE INVERSOR ---
            if sobra_modulos > 0:
                can.drawString(103, 528, "Inversor com capacidade")
                can.drawString(103, 512, f"de ampliar {sobra_modulos} {'Módulo' if sobra_modulos == 1 else 'Módulos'}")
            else:
                can.drawString(103, 528, "Inversor sem sobra")
                can.drawString(103, 512, "para ampliação")

            # GRÁFICO (PÁGINA 2)
            ordem_meses = ["JAN", "FEV", "MAR", "ABR", "MAI", "JUN", "JUL", "AGO", "SET", "OUT", "NOV", "DEZ"]
            prod_indices = {"JAN": 0.1350887, "FEV": 0.1110645, "MAR": 0.1031129, "ABR": 0.0817097, "MAI": 0.0722258,
                            "JUN": 0.0558870, "JUL": 0.0639952, "AGO": 0.0775323, "SET": 0.0876129, "OUT": 0.1100323,
                            "NOV": 0.1272935, "DEZ": 0.1454113}
            pot_t_w = float(qtd_mod) * float(pot_mod_w)

            # Histórico de Consumo
            met = st.session_state.get('metodo_consumo_key')
            fmt = st.session_state.get('formato_consumo_key')
            if met == "Leitura de PDF" or (met == "Manual" and fmt == "Histórico 12 Meses"):
                hist = st.session_state.get('historico_12_meses_key', {})
            else:
                m_c = float(st.session_state.get('consumo_final_key', 0.0))
                hist = {m: m_c for m in ordem_meses}

            larg_graf, alt_max, X_GRAF, Y_GRAF = 480, 210, 57.5, 255
            larg_mes = larg_graf / 12
            larg_bar = larg_mes * 0.35

            # Escala
            max_v = 1.0
            ger_man = st.session_state.get('geracao_manual_key', False)
            for m in ordem_meses:
                v_c = float(hist.get(m, 0))
                v_g = float(st.session_state.get(f"input_ger_{m}", 0)) if ger_man else pot_t_w * prod_indices[m]
                max_v = max(max_v, v_c, v_g)

            esc = alt_max / (max_v * 1.25)

            for i, mes in enumerate(ordem_meses):
                x_m = X_GRAF + (i * larg_mes)
                v_c = float(hist.get(mes, 0))
                can.setFillColorRGB(0, 0, 0.5)
                can.rect(x_m, Y_GRAF, larg_bar, v_c * esc, fill=1, stroke=0)

                v_g = float(st.session_state.get(f"input_ger_{mes}", 0)) if ger_man else pot_t_w * prod_indices[mes]
                can.setFillColorRGB(1, 0.65, 0)
                can.rect(x_m + larg_bar + 2, Y_GRAF, larg_bar, v_g * esc, fill=1, stroke=0)

                can.setFont("Helvetica-Bold", 6)
                can.setFillColorRGB(0, 0, 0)
                can.drawCentredString(x_m + (larg_bar / 2), Y_GRAF + (v_c * esc) + 3, f"{int(v_c)}")
                can.drawCentredString(x_m + larg_bar + 2 + (larg_bar / 2), Y_GRAF + (v_g * esc) + 3, f"{int(v_g)}")

                can.setFont("Helvetica-Bold", 8)
                can.drawCentredString(x_m + larg_bar, Y_GRAF - 12, mes)

            # --- LEGENDA DO GRÁFICO (ABAIXO E À ESQUERDA) ---
            y_legenda = Y_GRAF - 30
            can.setFont("Helvetica-Bold", 8)
            can.setFillColorRGB(0, 0, 0.5)
            can.rect(X_GRAF, y_legenda, 8, 8, fill=1, stroke=0)
            can.setFillColorRGB(0, 0, 0)
            can.drawString(X_GRAF + 12, y_legenda + 1, "Consumo")

            can.setFillColorRGB(1, 0.65, 0)
            can.rect(X_GRAF + 70, y_legenda, 8, 8, fill=1, stroke=0)
            can.setFillColorRGB(0, 0, 0)
            can.drawString(X_GRAF + 82, y_legenda + 1, "Geração")
        ...
        for page in new_pdf.pages: writer.add_page(page)
...
# DOWNLOAD
nome_cl = re.sub(r'[\\/*?:"<>|]', "", st.session_state.get('nome_cliente_key', 'Cliente'))
pot_final = ((float(st.session_state.get('qtd_modulos_final', 0)) * float(
    st.session_state.get('potencia_modulo_final', 0))) / 1000)
pdf_res = gerar_pdf_completo()
if pdf_res:
    st.download_button(label="Baixar Orçamento Completo", data=pdf_res,
                       file_name=f"Orçamento Lumo - {pot_final:.2f}KWP {nome_cl}.pdf", mime="application/pdf",
                       use_container_width=True)


# --- SEÇÃO 20 FUNÇÃO PARA GERAR O PDF DE CUSTOS EM PÁGINA ÚNICA (COMPLETA COM CIDADE) ---
def gerar_pdf_custos_detalhado():
    ...
    def draw_header():
        ...
        # Resgate de Nome e Cidade
        nome_cliente = st.session_state.get('nome_cliente_key', 'N/A')
        # Lógica para determinar a cidade exibida
        if st.session_state.get('cidade_key'):
            cidade_exibida = st.session_state.get('cidade_key')
        else:
            cidade_exibida = "Santa Cruz do Sul / Vera Cruz"

        can.setFont("Helvetica", 8)
        can.setFillColor(colors.black)
        can.drawString(50, y, f"Cliente: {nome_cliente} | Cidade: {cidade_exibida}")
        y -= 10
        can.drawString(50, y, f"Data: {pd.to_datetime('today').strftime('%d/%m/%Y %H:%M')}")
    ...
    modo_manual_ativo = st.session_state.get('orc_man_key') or st.session_state.get('orc_man_disabled')
    if modo_manual_ativo:
        potencia_exibida = st.session_state.get('potencia_key', 0)
    else:
        potencia_exibida = st.session_state.get('inversor_selecionado_final', 0)

    draw_item("Inversor:",
              f"{st.session_state.get('marca_key', 'N/A')} {potencia_exibida} kW")

    draw_item("Módulos:",
              f"{int(st.session_state.get('qtd_modulos_final', 0))}x{st.session_state.get('potencia_modulo_final', 0)}W")
    ...
    if geracao_manual_ativa:
        lista_geracao = [float(st.session_state.get(f"input_ger_{m}", 0.0)) for m in ms]
        m_g = sum(lista_geracao) / 12
    else:
        lista_geracao = [p_w * ind for ind in p_ind]
    ...
    max_valor_grafico = max(max(lista_geracao), m_c, 1.0)
    esc = 40.0 / max_valor_grafico
    ...
    dist_km = st.session_state.get('distancia_key', 0.0)
    v_deslocamento = dist_km * 1.00
    draw_item(f"Custo Deslocamento ({dist_km} km):", f"R$ {v_deslocamento:,.2f}")
