...
def buscar_imagem_logo(marca):
    # Definimos explicitamente o diretório como o local atual do script
    diretorio_logos = os.path.dirname(os.path.abspath(__file__)) if "__file__" in locals() else "."
    extensoes = ['.png', '.jpg', '.jpeg', '.PNG', '.JPG', '.JPEG']

    if not marca:
        return None

    # Limpa espaços extras e padroniza para maiúsculas
    marca_limpa = str(marca).strip().upper()

    for ext in extensoes:
        # Tenta localizar o arquivo (Ex: GROWATT.png)
        nome_arquivo = f"{marca_limpa}{ext}"
        caminho_teste = os.path.join(diretorio_logos, nome_arquivo)

        if os.path.exists(caminho_teste):
            return caminho_teste

    # Caso não encontre, exibe um aviso no Streamlit para te ajudar a debugar
    # st.warning(f"Logo não encontrada para a marca: {marca_limpa}")
    return None
# =================================================================
