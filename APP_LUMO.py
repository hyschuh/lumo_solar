...
def renderizar_pagina_3(can):
    ...
    # --- CÁLCULO DA MÉDIA DE GERAÇÃO (CORRIGIDO PARA MANUAL) ---
    geracao_manual_ativa = st.session_state.get('geracao_manual_key', False)

    if geracao_manual_ativa:
        # Soma os 12 meses preenchidos manualmente e tira a média para o PDF
        meses_ref = ["JAN", "FEV", "MAR", "ABR", "MAI", "JUN", "JUL", "AGO", "SET", "OUT", "NOV", "DEZ"]
        # AJUSTE: Lendo a chave correta da Seção 10 (input_ger_)
        soma_manual = sum([float(st.session_state.get(f"input_ger_{m}", 0)) for m in meses_ref])
        geracao_media_estimada = soma_manual / 12
    else:
        # Se não for manual, usa o cálculo automático original
    ...
    # Resgate de variáveis financeiras para o PDF (necessário estarem no session_state)
    ...
    valor_arredondado = st.session_state.get('valor_total_final_projeto', 0.0)
    ...
    meses_restantes = int(payback_meses % 12)
...
def gerar_pdf_completo():
    ...
    try:
        ...
        if os.path.exists(caminho_capa):
            ...
            can_capa.setFillColorRGB(0.0, 0.176, 0.506)  # Azul Marinho
        ...
        if os.path.exists(caminho_fundo_p2):
            ...
            # Módulos
            curr_x_m = X_ALINHAMENTO_TECNICO
            ...
            if sobra_modulos > 0:
                can.drawString(X_ALINHAMENTO_TECNICO, 528, "Inversor com capacidade")
                can.drawString(X_ALINHAMENTO_TECNICO, 512,
                               f"de ampliar {sobra_modulos} {'Módulo' if sobra_modulos == 1 else 'Módulos'}")

            # --- Lógica do Gráfico (Página 2) ---
            ordem_meses = ["JAN", "FEV", "MAR", "ABR", "MAI", "JUN", "JUL", "AGO", "SET", "OUT", "NOV", "DEZ"]
            ...
            X_ALINHAMENTO_GRAFICO = 57.5
            Y_BASE_GRAFICO = 240
            largura_mes = largura_grafico / 12
            largura_barra = largura_mes * 0.35

            # --- AJUSTE: CÁLCULO DA ESCALA MÁXIMA (CONSIDERANDO MANUAL) ---
            max_valor = 1.0
            geracao_manual_ativa = st.session_state.get('geracao_manual_key', False)

            for m in ordem_meses:
                v_consumo = float(hist.get(m, media_c))
                if geracao_manual_ativa:
                    # AJUSTE: Lendo a chave correta da Seção 10 (input_ger_)
                    v_geracao = float(st.session_state.get(f"input_ger_{m}", 0))
                else:
                    v_geracao = pot_total_w * prod_mensal[m]
                max_valor = max(max_valor, v_consumo, v_geracao)

            escala = altura_max_barra / (max_valor * 1.15)

            # Desenho das Barras
            for i, mes in enumerate(ordem_meses):
                x_mes = X_ALINHAMENTO_GRAFICO + (i * largura_mes)

                # Barra de Consumo (Azul)
                ...
                # --- AJUSTE: GERAÇÃO MANUAL NO GRÁFICO ---
                if geracao_manual_ativa:
                    # AJUSTE: Lendo a chave correta da Seção 10 (input_ger_)
                    val_g = float(st.session_state.get(f"input_ger_{mes}", 0))
                else:
                    val_g = pot_total_w * prod_mensal[mes]

                h_g = val_g * escala
                can.setFillColorRGB(1, 0.65, 0)  # Laranja
...
# --- EXECUÇÃO E DOWNLOAD (ESTRUTURA REVISADA PARA EVITAR ERROS) ---
nome_cliente_raw = st.session_state.get('nome_cliente_key', 'Cliente').strip()
nome_cliente_limpo = re.sub(r'[\\/*?:"<>|]', "", nome_cliente_raw)

# Cálculos rápidos para o nome do arquivo
try:
    qtd_f = float(st.session_state.get('qtd_modulos_final', 0))
    pot_f = float(st.session_state.get('potencia_modulo_final', 0))
    pot_kwp_final = (qtd_f * pot_f) / 1000
except:
    pot_kwp_final = 0.0

nome_do_arquivo = f"Orçamento Lumo - {pot_kwp_final:.2f}KWP {nome_cliente_limpo}.pdf"

# Geramos o PDF fora do botão para garantir estabilidade
...
if pdf_finalizado:
    ...
    st.error("Ocorreu um erro ao preparar o arquivo PDF. Verifique os dados inseridos.")
